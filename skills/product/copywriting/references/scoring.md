...
The shared rubric for copywriting evaluation: severity, score bands, and the report shape that `critique.md` and `audit.md` both produce.

## When to Use

Composed by `critique.md` (quality verdict on a draft or direction) and `audit.md` (ship-readiness report on `copy.yaml`). It carries the numbers and the template; the modes supply the findings. Not a direct trigger.
...
Unsure between two levels? Ask "would this cost a conversion or mislead the reader?" If yes, it is at least P1.

`anti-patterns.md` rules carry an `error | warning` severity. Map them into
findings: **error → P0/P1** (P0 if it misleads or blocks, else P1); **warning → P2/P3** (P2 if a reader would notice, else P3).
...
Score Clarity, Voice consistency, So what, Prove it, Specificity, Heightened emotion, and Zero risk (defined in [editing-sweeps.md](editing-sweeps.md)) 0–4 each.
...
Score Readability, Claim integrity, Conversion readiness, Microcopy correctness, and Anti-pattern density 0–4 each.
...
- A weak critique axis → the matching `refresh` sweep (Clarity → clarity pass, Prove it → prove it pass, and so on); re-critique after.
