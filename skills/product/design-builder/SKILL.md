...
description: >-
  ...
  visual identity, define page composition, screen flow, or commerce
  surfaces, preview and refine variants. Use
...
tokens plus numbered prose sections that narrate them, define page
composition or screen flow in a separate structure artifact, preview,
refine. Content comes from `copy.yaml`, read here as the content
payload — never authored or owned by this skill.

## Workflow

```text
design --> structure --> preview
  |           |             ^__|
  |           |           (tune loop)
  v           v
...
one you need. Content (`copy.yaml`) is read as an upstream input, not
produced here. Brownfield drift after handoff → `reconcile.md`.
...
- **Greenfield** — zero to `DESIGN.md` + `structure.md` from raw inputs
  (URL, images, brief, codebase, design-tool file); content comes from
  `copy.yaml`. Default path; runs design → structure → preview.
...
`docs/design/structure.md`. `copy.yaml` is content-only and authored
upstream — read here, never written. `reconcile.md` patches DESIGN.md from a
