...
**Existing DESIGN.md.** Anchor-only — skill reads the YAML frontmatter of `.agents/design/DESIGN.md` already populated by `inputs.md` as the anchor's authored state.
...
- Compose into one DESIGN.md at `.agents/design/DESIGN.md`, following the template owned by [inputs.md](inputs.md). Emit the YAML frontmatter first (authoritative tokens), then the prose body that narrates them.
