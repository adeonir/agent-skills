...
Manages the delivery lifecycle with tracker-first integration and markdown fallback. Plan epics, track stories, report bugs, file tasks, and push to a tracker (via MCP or CLI) when configured.
...
`sync.md` is also auto-loaded by core refs (epic, story, task, bug) after the artifact is saved when `epic-tracker.kind` is set and not `none`.
...
A single file holding both a story and the bugs it spawned, or an epic mixed with its implementation plan, makes status updates and tracker pushes ambiguous. One file = one artifact type. Bugs go to their own file (in the epic folder or `standalone/`); tasks go to the epic folder or `standalone/`.
