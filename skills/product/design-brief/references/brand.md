...
- **Typography** — a characterful display paired with a refined body; real weight and size contrast (display ≥3× body); fluid `clamp()` headings. Reflex defaults (Inter, Roboto, system) are a tell on a brand surface.
