...
- **Typography** — from `typography.*`. Each role becomes related properties (`--font-display-family`, `--font-display-size`, `--font-display-weight`, `--font-display-style`, ...) — one per field the role defines, so `fontStyle` (italic), `lineHeight`, `letterSpacing`, `fontFeature`, and `fontVariation` all emit and the specimen renders as authored. Role keys map 1:1 to kebab-case prefixes.
