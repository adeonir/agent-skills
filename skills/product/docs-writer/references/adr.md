...
For a new record, read `<this-skill>/assets/adr.template.md`, copy its exact structure, remove every comment, and replace every square-bracket slot. For an existing record, preserve all content outside the requested update and use the template only to check structure.
