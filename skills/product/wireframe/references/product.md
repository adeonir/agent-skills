...
When the surface serves a task — the user is mid-work and the arrangement gets them through it: dashboard, settings, data-table, form, onboarding, the checkout / account flow of a storefront. Structure follows the task, not a narrative.

## When to Use

Read by `create.md` and `validate.md` when the surface is product — to bias its arrangement. Name the register first, then read only the matching file: product here, or [brand.md](brand.md) when the surface communicates. Not a direct trigger.
...
**Register** (brand vs product) is the posture — two values. **Surface** is the granular type the plan arranges (dashboard, settings, data-table, onboarding…). A surface sits under a register, but the register is not a finer surface list. Surfaces under this register:
