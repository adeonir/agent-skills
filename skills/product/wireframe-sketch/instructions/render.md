...
scale, token, or interactivity. It carries no visual identity, so the same
render holds under any design. It never reads or infers styling from anywhere.
...
|--------|----------|----------|
| `html` | simple or dense screens; the general-purpose view | `docs/design/wireframe.html` (committed) |
...
writes `docs/design/wireframe.html` and prints the ASCII sketch and the Mermaid
flow to stdout.

### Step 2: Place the output

- **`wireframe.html`** — already written next to `WIREFRAME.md`. It opens
  directly in a browser; no server. It carries a `generated — do not edit`
  header: it is derived, never a source. Hand edits belong in `WIREFRAME.md`,
  then re-render.
