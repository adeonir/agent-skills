...
- **critique** reads `PRODUCT.md` for posture and anti-references, plus the chosen variant HTML in `.artifacts/` (render's output).
- **audit** reads a running UI (required), `PRODUCT.md` for posture and anti-references, plus optional source; inputs degrade gracefully.
