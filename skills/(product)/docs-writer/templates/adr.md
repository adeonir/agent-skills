---
name: {{adr-name}}
type: adr
number: {{number}}
status: proposed
created: {{YYYY-MM-DD}}
decision_makers: {{names}}
supersedes: {{ADR-n, or "None"}}
---

# ADR-{{number}}: {{Title}}

## Context

{{What is the issue that we're seeing that is motivating this decision or change? Describe the forces at play -- technical, business, team, constraints.}}

## Decision Drivers

1. {{Force or requirement that shaped the decision}}
2. {{Another force or constraint}}
3. {{Additional driver influencing the choice}}

## Options Considered

### Option 1: {{Name}}

{{Description of this approach}}

**Pros:**
- {{Advantage of this option}}
- {{Another benefit}}

**Cons:**
- {{Disadvantage or risk}}
- {{Another downside}}

### Option 2: {{Name}}

{{Description of this approach}}

**Pros:**
- {{Advantage of this option}}
- {{Another benefit}}

**Cons:**
- {{Disadvantage or risk}}
- {{Another downside}}

### Option 3: {{Name}} *(if applicable)*

{{Description of this approach}}

**Pros:**
- {{Advantage of this option}}

**Cons:**
- {{Disadvantage or risk}}

## Decision

{{Which option was chosen -- state clearly in one sentence, then elaborate on the rationale and what factors were most important.}}

## Consequences

### Positive

- {{Benefit of this decision}}
- {{Another positive outcome}}

### Negative

- {{Downside or trade-off accepted}}
- {{Another negative consequence}}

### Neutral

- {{Side effect that is neither positive nor negative}}

### Risks

- {{Risk that remains even after the decision, and how to monitor it}}

## Follow-Up Actions

- [ ] {{Action required to implement or communicate this decision}}
- [ ] {{Another follow-up task}}

## References

- {{Link to RFC, discussion thread, or related ADR}}
- {{Link to relevant documentation or prior art}}
