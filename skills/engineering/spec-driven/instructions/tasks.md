...
5. **Self-check** — run `python3 ${CLAUDE_SKILL_DIR}/scripts/lint_artifact.py tasks .artifacts/specs/{slug}` and fix every line it reports, then read for what it cannot: every AC's task and test actually cover it; no task depends on a later task without a note; tests are co-located with the code they cover, never deferred. Watch for scope breaking the sizing — see the safety valve in [sizing.md](../references/sizing.md).
