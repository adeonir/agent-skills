...
6. **Approval gate** — present the path of `tasks.md`, the task count, the commit count the boundaries produce, and the wave sequence the user may reorder, then ask *"Move to implement?"* Name anything the run wrote that the project does not ignore and suggest the commit — see [memory.md](../references/memory.md).
