...
6. **Peer check** — Medium and up: the agent that built the breakdown cannot clear it, so dispatch a subagent with no conversation history, handed `tasks.md`, `spec.md`, and `design.md` — never a summary of how the breakdown was built or a claim that it holds, since a delivered conclusion anchors the reader toward agreement. It reads the task list against its own contract: the template's MUST-NOT, the Coverage Matrix against the spec's ACs, story contiguity and dependency order, a task that introduces a decision instead of sequencing one, and a gate that would still pass with the logic removed. It reports findings and never edits the file. Fix each inline, then re-check what changed.
