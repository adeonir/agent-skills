...
6. **Approval gate** — present the path of `tasks.md`, the task count, and the wave sequence the user may reorder, then ask *"Move to implement?"* Name anything the run wrote that the project does not ignore and suggest the commit — see [memory.md](../references/memory.md).
...
- **Test:** `[file]` — `[runner test case]` <!-- required when Covers is present; `none — [what no runner reaches]` when the project has no runner for the outcome -->
