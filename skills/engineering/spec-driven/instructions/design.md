...
5. **Research** — only when the knowledge chain (codebase → project docs → a docs MCP when available, e.g. Context7 → web) is exhausted without an answer. Inline by default; a subagent only for a large or independent topic. Cache to `.artifacts/research/{topic}.md`. Knowing the syntax is not knowing the environment accepts it — the spike discipline in step 3 applies regardless of whether research ran.
