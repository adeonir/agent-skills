...
3. **Exploration** — dispatch a light subagent with only the relevant area as input; it returns a short structured summary of patterns, files to touch, and risks. Exploration challenges the plan; it does not confirm it — never stop at the first pattern that matches the input's own words. When a placement or trigger has ≥2 viable homes, **enumerate the entry points at that layer** before choosing, and **trace runtime provenance, not just structure** — how does neighboring data of the same kind already arrive here on a real run? The main agent judges and fills the template. `file:line` citations only on load-bearing claims (decisions, risks, reuse), and only when handy.
...
7. **Self-check** — boundaries hold (nothing from spec leaked in, nothing from tasks leaked in — see [discriminator.md](../references/discriminator.md)); any decision conflicting with `CONTEXT.md` is conformed or explicitly superseded, never ignored; every placement decision can name what it ruled out — if it cannot, the exploration is unfinished, not the design.
...
A placement or trigger with ≥2 viable homes is a Decisions row, not a silent mechanical pick: record the choice and name the rejected alternative in its Rationale, even when one home looks obvious.
