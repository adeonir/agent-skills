...
4. **Exploration** — dispatch a light subagent with the spec and the return contract below, never a narrowed area: narrowing the area decides where the feature lives before anyone read the code, and turns the exploration into a confirmation of that choice. The main agent judges what comes back and fills the template.
   ...
   - **Semantic contract inventory** — before choosing or reusing any field, token, label, state, schema property, or renderer mapping, search the project rules, schemas, content, renderers, styles, and tests for every existing declaration and usage of that name. Record the established meaning, its current mapping, and any collision in the `## Semantic Contract Inventory` table. A rule citation alone is insufficient: the inventory must include evidence from the declaration and at least one real usage when both exist. If an existing name carries a different meaning, do not redefine it in design; choose an unambiguous name, or route back to specify when the upstream contract fixed the conflicting name.
...
|-----------|-----------------------------|--------------------|------------------------|----------|--------|
| [semantic name, token, field, state, or `none`] | [project evidence, or `none`] | [current meaning, or `none`] | [current/proposed mapping, or `none`] | [collision, or `none`] | [file/rule/symbol, or `default`] |
