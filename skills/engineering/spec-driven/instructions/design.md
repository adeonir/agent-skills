...
3. **Land pending questions** — sweep the spec's `## Open Questions` for `@ design` clauses. No `@ design` line may remain unresolved when the template is filled; each resolution lands as a Decisions row or a Risks entry.
   ...
   - **`(confirm @ design)`** — joins the questions batched for the user before writing. A failed verification never becomes a task: record it as a precondition in Risks & Concerns, or set `STATE.md` to `Phase: specify` and `Next: specify` when it breaks the WHAT.
