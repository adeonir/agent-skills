...
1. **Confirm the gate** — read `.artifacts/STATE.md ## Progress` before trusting an existing `validation.md`. If `Next` directs design, tasks, implementation, or audit work, stop and run it: an earlier PASS predates that pending work. Then confirm `spec.md user-facing: true` and current audit status PASS. Otherwise stop; there is nothing to validate. Set `Phase` to `validate`.
