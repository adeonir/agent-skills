...
1. **Resolve feature** — find `.artifacts/specs/{slug}/`, read its `STATE.md ## Progress`, and confirm `spec.md` is `ready` and `tasks.md` is `done`. If `Findings` names a report, stop and run `tasks` before validating again. If `Phase` points to `specify`, `design`, or `tasks`, stop and report that phase. If a prerequisite is not ready, stop and report that phase. Set the active feature's `STATE.md ## Progress` `Phase` to `validate`.
