...
1. **Resolve feature** — find `.artifacts/specs/{slug}/`, read `.artifacts/STATE.md ## Progress`, and confirm `spec.md` is `ready` and `tasks.md` is `done`. If `Findings` names a report, stop and run `tasks` before validating again. If a prerequisite is not ready, stop and report that phase. Set `STATE.md ## Progress` `Phase` to `validate`.
...
MUST NOT contain: code fixes, changes to `spec.md` or `tasks.md` status, or audit findings. Record validate findings here and route correction tasks through `STATE.md` and the `tasks` phase.
