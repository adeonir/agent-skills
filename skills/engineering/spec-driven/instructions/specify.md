...
7. **Write the Divergences table** — record where the spec and the artifact it was specced from disagree now. The seed has readers who never open the spec, so once the two stop agreeing, whoever reads only the seed expects something else. A line opens in one of three directions: **Added**, the spec asserts what the seed does not; **Dropped**, the seed promises what no AC covers; **Loosened**, the seed asks for more than its story's benefit needs and the spec asserts the weaker observable. Point the `AC` column at the criterion carrying the divergence, and leave it empty on a `Dropped` line, which by definition no criterion carries. Read the seed against the table already on disk: preserve the id and status of each difference that still exists, open a new one as `open`, and remove a row once the seed and spec agree. Only the user changes `open` to `accepted`; acceptance keeps a known difference in the table while it exists, and alignment removes it like any other row. Never on a prompt-seeded spec (`sources: []`) — there is no seed to diverge from, and the section carries `none`.
...
10. **Approval gate** — present name and scope, the seed (the issue or document `sources` records, or "prompt-seeded — `sources` empty"), 2-3 bullets of what changes, every AC a calibration loosened or confirmed strict, the open questions that survive the spec (`@ design` and `[deferrable]` lines), and every `open` row of Divergences, then ask *"Move to design?"* Never hide the surviving pendencies — an open `DV-N` says the seed no longer matches what will be built, so it is presented as that, with the correction landing on the seed, never folded into the list unremarked.
    ...
    On a re-entry, compare the previous and rewritten contract before the gate. When only criterion IDs changed, update the `AC` column of `design.md ## Requirements Traceability` and `tasks.md ## Coverage Matrix` in the same pass; the phases do not re-run. When observable contract changed — a Goal, story benefit, criterion condition, or criterion outcome was added, removed, tightened, or loosened — keep downstream artifacts as context, set each existing `design.md` and `tasks.md` to `status: draft`, and set the active feature's `STATE.md ## Progress` to `Phase: design` and `Next: rerun design for {changed ids or contract clauses}`. Otherwise preserve the progress already in the feature's state. Present which path the re-entry took and, for a contract change, what moved.
11. **Update the active feature's `STATE.md ## Progress`** — for a new spec, point to design. On re-entry, preserve the existing progress after an unchanged or id-only pass; after a contract change, keep the design rerun written in step 10. See [memory.md](../references/memory.md).
