...
5. **Write `spec.md`** — fill the template below from resolved inputs only; infer `branch` from the content, never ask. Author acceptance criteria per [acceptance-criteria.md](../references/acceptance-criteria.md) — one fenced `gherkin` scenario per criterion, `Scenario` for a single case and `Scenario Outline` + `Examples` for a parametrized one. Slice each `S-N` as one vertical slice per [slicing.md](../references/slicing.md). Record each decision as a settled fact — an AC, a Goal, or a Non-Goal — never the clarification exchange that produced it ("we discussed", "you chose", "as decided above"): a reader sees the contract, not how it was reached. A default advanced without confirmation is an `ASM-N` row, not a settled fact. A gap with no safe default is an `OQ-N` row. Frontmatter `sources` records only the seed by contract, not by judgment: the tracker issue (story, task, or bug) or document the spec is seeded from enters `sources` under the URL or id it arrived as. Do not copy links from the seed's `References` into `sources`; when the spec consumes one, carry its title and URL to `## References`. Write the seed as one bullet under `sources:`. A single `- none` item means one thing only: the spec is prompt-seeded, with no issue or document behind it.
6. **Write the pending tables and Divergences** — record each default the spec rests on as an `ASM-N`, each question without a safe default as an `OQ-N`, and each obligation the seed and the spec no longer agree on as a `DV-N`. The three sections record what survived discovery, never what a reread of the seed turns up: a spec whose discovery settled everything writes `none` in all three. An assumption names what in the spec rests on it — a criterion, a Goal, or a stated fact; a default nothing rests on is not an assumption, and a matter of process or authoring never is. A divergence is a difference of obligation, not of wording: the seed has readers who never open the spec, so it opens where whoever reads only the seed would expect something else. It opens in one of three directions: **Added**, the spec obligates what the seed does not promise; **Dropped**, the seed promises what no AC covers; **Loosened**, the seed asks for more than its story's benefit needs and the spec asserts the weaker observable. Refining what the seed already intends is not a divergence, and neither is behavior carried by an artifact the seed references. Read the existing tables before writing: preserve the id and status of each `ASM-N`, `OQ-N`, and `DV-N` that still exists, continue numbering from the highest id, and mark a row resolved only through its defined status transition. Never create a divergence on a prompt-seeded spec (`sources` carrying `- none`) — there is no seed to diverge from, and that section carries `none`.
...
8. **Approval gate** — present the path of `spec.md`, one or two sentences of what the feature does, and every `open` `ASM-N`, `OQ-N`, and row of Divergences, then ask *"Move to design?"* Never hide the surviving pendencies — an open `DV-N` says the seed no longer matches what will be built, so it is presented as that, with the correction landing on the seed, never folded into the list unremarked. Name anything the run wrote that the project does not ignore and suggest the commit — see [memory.md](../references/memory.md).
...
9. **Update the feature's `STATE.md ## Progress`** — for a new or existing spec, point to `Phase: design` and `Next: design`. When a report routed the work here, clear the consumed source from `Findings` and keep any other source. See [memory.md](../references/memory.md).
...
## Assumptions
<!-- the defaults the spec rests on, each naming what rests on it. `none` when discovery
...
## Divergences
<!-- where the spec and the seed disagree on an obligation, with the correction landing on the
     seed. Refining what the seed intends is not a disagreement. Opens Added / Dropped / Loosened;
     the AC column names the criterion carrying it, and stays empty on a Dropped line. Remove a
     row once seed and spec agree; only the user writes accepted, which keeps a known difference
     while it exists. `none` when no current divergence exists. -->
| ID | AC | Status | Divergence |
|----|----|--------|------------|
| DV-1 | AC-1.1 | open | Added: [what the spec obligates and the seed does not promise] |
