...
11. **Update the active feature's `STATE.md ## Progress`** — for a new or existing spec, point to `Phase: design` and `Next: design`. When a report routed the work here, clear the consumed source from `Findings` and keep any other source. See [memory.md](../references/memory.md).
