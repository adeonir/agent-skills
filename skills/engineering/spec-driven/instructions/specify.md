...
5. **Discuss** — when a gray area is load-bearing (changes Goals, ACs, or the approach); a safe default does not license skipping, and when in doubt whether a fork is load-bearing, ask. Batch the questions and resolve them before writing the spec body. An answer resolves only when it carries a concrete choice or content — "Other" with no elaboration, a counter-question, or a partial answer keeps the question open: follow up until it lands, never advance on a default. `discuss.md` is written only at Complex; otherwise fold the resolution into the spec. See [discovery.md](../references/discovery.md).
