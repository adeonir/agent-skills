...
10. **Approval gate** — present name and scope, the seed (the issue or document `sources` records, or "prompt-seeded — `sources` empty"), 2-3 bullets of what changes, every AC a calibration loosened or confirmed strict, the open questions that survive the spec (`@ design` and `[deferrable]` lines), and every `open` row of Divergences, then ask *"Move to design?"* Never hide the surviving pendencies — an open `DV-N` says the seed no longer matches what will be built, so it is presented as that, with the correction landing on the seed, never folded into the list unremarked.
    ...
    On a re-entry, the gate also names what this pass invalidated downstream, since `design.md` and `tasks.md` are read by their own phases and nothing else re-reads them. An id-only change — criteria renumbered, contract unchanged — needs the `AC` column of `## Requirements Traceability` and `## Coverage Matrix` corrected, and no phase re-run. A contract change needs the phases themselves: an added criterion has no component, no task, and no test; a removed one may leave a task serving nothing; a loosened one may have been the only reason a design decision was taken. Name which of the two this pass was, and for a contract change, which criteria moved.
