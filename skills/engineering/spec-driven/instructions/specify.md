...
7. **Write the Divergences table** — record where the spec and the artifact it was specced from no longer agree. The seed has readers who never open the spec, so once the two stop agreeing, whoever reads only the seed expects something else. A line opens in one of three directions: **Added**, the spec asserts what the seed does not; **Dropped**, the seed promises what no AC covers; **Loosened**, the seed asks for more than its story's benefit needs and the spec asserts the weaker observable. Point the `AC` column at the criterion carrying the divergence, and leave it empty on a `Dropped` line, which by definition no criterion carries. Read the seed against the table already on disk: keep every line it holds, open a new one as `open`, and turn to `resolved` the line whose divergence the seed no longer holds — which is the run after the seed is corrected. Only the user writes `accepted`, and that mark stands on every run after. The rewrite is from the template, so the record survives only because this step preserves it. Never on a prompt-seeded spec (`sources: []`) — there is no seed to diverge from, and the section carries `none`.
8. **Self-check** — run `python3 ${CLAUDE_SKILL_DIR}/scripts/lint_artifact.py spec .artifacts/specs/{slug}` and fix every line it reports; it settles structure, presence, scenario form, and cross-file references, so the passes below read only for what it cannot. Then, each failure fixed before saving:
   ...
   - **Seed record** — every divergence this pass opened carries its `DV-N` row, every row the previous spec held is still here, and each one still names a direction the seed actually diverges in. An issue-seeded or doc-seeded spec whose seed is absent from `sources` fails the check — add it before saving. A mention is not a seed: an issue or document cited during discovery as context or dependency stays out of `sources` unless the spec is specced from it.
...
10. **Approval gate** — present name and scope, the seed (the issue or document `sources` records, or "prompt-seeded — `sources` empty"), 2-3 bullets of what changes, every AC a calibration loosened or confirmed strict, the open questions that survive the spec (`@ design` and `[deferrable]` lines), and every `open` row of Divergences, then ask *"Move to design?"* Never hide the surviving pendencies — an open `DV-N` says the seed no longer matches what will be built, so it is presented as that, with the correction landing on the seed, never folded into the list unremarked.

    On a re-entry, compare the previous and rewritten contract before the gate. When only criterion ids changed, update the `AC` column of `design.md ## Requirements Traceability` and `tasks.md ## Coverage Matrix` in the same pass; the phases do not re-run. When observable contract changed — a Goal, story benefit, criterion condition, or criterion outcome was added, removed, tightened, or loosened — leave the downstream artifacts on disk as context and set `STATE.md ## Progress` to `Phase: design` and `Next: rerun design for {changed ids or contract clauses}`. Otherwise preserve the progress already in `STATE.md`. Present which path the re-entry took and, for a contract change, what moved.
11. **Update `STATE.md ## Progress`** — for a new spec, point to design. On re-entry, preserve the existing progress after an unchanged or id-only pass; after a contract change, keep the design rerun written in step 10. See [memory.md](../references/memory.md).
...
<!-- only unresolved items live here: a resolved question becomes spec content (AC, Goal, Non-Goal)
     ...
     Where the spec and its seed disagree, the record is a Divergences row, never a line here. -->
...
## Divergences
<!-- where the spec and the seed stopped agreeing, with the correction landing on the seed. Opens
     Added / Dropped / Loosened; the AC column names the criterion carrying it, and stays empty on
     a Dropped line. A row turns resolved on the run that finds its divergence gone; only the user
     writes accepted. `none` on a prompt-seeded spec — there is no seed to diverge from. -->
