...
7. **Approval gate** — present the path of `spec.md`, one or two sentences of what the feature does, and every `open` `ASM-N`, `OQ-N`, and row of Divergences, then ask *"Move to design?"* Never hide the surviving pendencies — an open `DV-N` says the seed no longer matches what will be built, so it is presented as that, with the correction landing on the seed, never folded into the list unremarked. Name anything the run wrote outside the ignored folders and suggest the commit — see [memory.md](../references/memory.md).
...
8. **Update the feature's `STATE.md ## Progress`** — for a new or existing spec, point to `Phase: design` and `Next: design`. When a report routed the work here, clear the consumed source from `Findings` and keep any other source. See [memory.md](../references/memory.md).
