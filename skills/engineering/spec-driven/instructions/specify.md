...
8. **Self-check** — run `python3 ${CLAUDE_SKILL_DIR}/scripts/lint_artifact.py spec .artifacts/specs/{slug}` and fix every line it reports; it settles structure, presence, and cross-file references, so the passes below read only for what it cannot. Then, each failure fixed before saving:
