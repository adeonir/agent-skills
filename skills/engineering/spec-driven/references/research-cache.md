...
A finding is cached so the next design reads it instead of re-deriving it: no re-fetching a doc, no re-running a command against the environment. Reuse is the only reason the file exists — a finding scoped so tightly to one feature that nothing else could ask it again is not cached at all; it lands in `design.md` and dies with the feature.

A documentary finding (what the docs state for the installed version) and an observed finding (what a command proved against this environment) share one file, because they answer the same shape of question. Only their `source` differs.

The command that produced an observation is never cached. What survives is the observation itself.
...
- **Basis diverges** — every finding in the file is void. Do not weigh it, do not treat it as a hint, do not "probably still true" it. Re-verify with the cheapest observation that answers the question and overwrite the file with the new observation and the new basis.
