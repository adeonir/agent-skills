...
| **Complex** | Ambiguity in the problem itself | full; ambiguity closed | full + approaches + research | full | subagent | optional, `user-facing` only | optional subagent |

Small does not produce a spec and does not run the pipeline: one-liner → branch → inline implement → inline verify.

Every artifact closes on its own self-check plus the linter, at every scope. The independent reading is the audit, and what makes it independent is the evidence it reaches: it reads the diff and the tests, which no phase that wrote an artifact ever did. Every claim an artifact makes is re-read against running code — by the safety valve when a task turns out to carry a decision, and by the audit's discrimination sensor when a gate would pass with the logic removed.
...
- **Large → Complex** — the design gains approaches.
