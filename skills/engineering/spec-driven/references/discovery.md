...
- **Seed prose that warns rather than obliges** — a seed's cautions, rabbit holes, and out-of-bounds notes carry content the Definition of Done does not, and it lands in the spec by kind: a scope guard becomes a Non-Goal with its reason, a boundary the system does not owe becomes an Edge Case, and one that settles nothing becomes a `[deferrable]` open question. A warning is not an obligation — it never mints an AC on its own; where the seed genuinely obliges something there, it is a seed obligation and reaches an AC through the same walk every other one takes.
