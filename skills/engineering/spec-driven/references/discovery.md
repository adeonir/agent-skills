...
- **Seed prose that warns rather than obliges** — a seed's cautions, rabbit holes, and out-of-bounds notes carry content the Definition of Done does not. Each enters as a claim, not as a finding: check it against what the codebase and the seed's own references show, and a warning nothing supports lands nowhere — sorting it by kind is what launders it into the spec, since every destination below reads as sourced. A warning that survives the check lands by kind: a scope guard becomes a Non-Goal with its reason, a boundary the system does not owe becomes an Edge Case, and one that settles nothing becomes a `[deferrable]` open question. A warning is not an obligation — it never mints an AC on its own; where the seed genuinely obliges something there, it is a seed obligation and reaches an AC through the same walk every other one takes. A surviving warning that is also a mechanism fork routes by what the spec owes, not by where it came from: it stays `[deferrable]` while nothing the spec asserts depends on the answer, and becomes an `[assumption]` closed with `(confirm @ design)` only once something does — the warning alone never mints the confirm, since it commits the spec to nothing.
