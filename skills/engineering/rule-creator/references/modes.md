...
1. Check `.claude/rules/` exists. If not, output "No rules defined in this project." and exit.
...
1. Resolve target. The user said "edit rule X". X may be a filename (`testing.md`), a topic (`testing`), or a rule title (`Test File Placement`).
   - Filename match → use directly.
   - Topic match → use `<topic>.md`.
   - Rule title match → grep H2 headings across `.claude/rules/`, pick the file that contains it.
